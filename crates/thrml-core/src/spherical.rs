...
impl SphericalCoords {
    ...
    pub fn from_cartesian(cart: Tensor<WgpuBackend, 2>) -> Self {
        ...
        let r =
            (x.clone().powf_scalar(2.0) + y.clone().powf_scalar(2.0) + z.clone().powf_scalar(2.0))
                .sqrt();
    ...
    pub fn geodesic_distance_to(&self, other_theta: f32, other_phi: f32) -> Tensor<WgpuBackend, 1> {
        let device = self.device();
        ...
        let cos_d =
            sin_theta.mul_scalar(other_sin_t) * cos_phi_diff + cos_theta.mul_scalar(other_cos_t);
        let cos_d_clamped = cos_d.clamp(-1.0, 1.0);
