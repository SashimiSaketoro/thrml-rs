...
fn test_model_backend_selection() {
    ...
    assert!(uses_cpu_for_ising, "Apple Silicon should route IsingSampling to CPU");
    assert!(uses_cpu_for_gradient, "Apple Silicon should route GradientCompute to CPU");
}
...
fn test_nvidia_consumer_routing() {
    ...
    assert!(uses_cpu_for_ising, "Consumer NVIDIA should route IsingSampling to CPU");
}
...
fn test_gpu_only_routing() {
    ...
    assert!(!uses_cpu_for_gradient, "GpuOnly should not route GradientCompute to CPU");
    assert!(!uses_cpu_for_energy, "GpuOnly should not route EnergyCompute to CPU");
    assert!(!uses_cpu_for_ising, "GpuOnly should not route IsingSampling to CPU");
}
...
fn test_hpc_gpu_routing() {
    ...
    #[cfg(not(feature = "cuda"))]
    {
        ...
        assert!(uses_cpu_for_ising, "HPC without CUDA should route IsingSampling to CPU");
    }
...
fn test_runtime_policy_tiers() {
    ...
    assert!(!nvidia_consumer.is_hpc_tier(), "Consumer NVIDIA is not HPC tier");
...
fn test_precision_profiles() {
    ...
    assert!(matches!(nvidia_consumer.profile, PrecisionProfile::GpuMixed));
