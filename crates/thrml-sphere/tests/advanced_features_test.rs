...
fn test_hypergraph_spring_forces() {
    ...
    println!(
        "Created hypergraph with {} nodes, edges: chain(0-4), cluster(5-7)",
        n
    );
    ...
    let hypergraph_ebm =
        HypergraphEBM::from_sidecar(&sidecar, spring_constant, coherence_weight, &device);
    ...
    println!(
        "Chain nodes (0-4) avg force magnitude: {:.4}",
        chain_force_avg
    );
    println!(
        "Unconnected nodes (8-19) avg force magnitude: {:.4}",
        unconnected_force_avg
    );

    assert!(
        chain_force_avg > 1e-6,
        "Connected nodes should have non-zero spring forces"
    );
    assert!(
        unconnected_force_avg < 1e-6,
        "Unconnected nodes should have ~zero spring forces"
    );
...
fn test_hypergraph_coherence_energy() {
    ...
    if let Some(coh_energy) = hypergraph_ebm.coherence_energy(&coords, max_radius) {
        ...
        assert!(
            high_coh_energy > low_coh_energy,
...
fn test_lasing_dynamics_amplification() {
    ...
    println!(
        "  High coherence (node 19, coh=1.0): r={:.2}",
        final_r[n - 1]
    );

    // High coherence should have smallest radius
    assert!(
        final_r[n - 1] < final_r[10],
        "Highest coherence should have smallest radius"
    );
    assert!(
        final_r[10] < final_r[0],
        "Mid coherence should be between high and low"
    );
    assert!(
        final_r[0] > mid_radius * 0.5,
        "Low coherence should grow outward from initial: {} > {}",
        final_r[0],
        mid_radius * 0.5
    );
    assert!(
        final_r[n - 1] < mid_radius,
...
fn test_combined_lasing_langevin() {
    ...
    let config = SphereConfig::from(ScaleProfile::Dev).with_steps(30); // Reduced for test speed
    ...
    let combined = LasingLangevinOptimizer::new(
        lasing, 5,  // 5 langevin steps per lasing step
        20, // 20 outer iterations
    );

    let init_coords = ebm.init_coords(&device);
    let init_r: Vec<f32> = init_coords
        .r
        .clone()
        .into_data()
        .to_vec()
        .expect("init r to vec");
    ...
    assert!(
        min_r >= 20.0 && max_r <= 600.0,
        "Radii should be within reasonable bounds"
    );

    println!(
        "\n✓ Combined lasing+langevin test PASSED (correlation: {:.3})!",
        correlation
    );
}
...
fn test_sparse_similarity_topk() {
    ...
    println!(
        "Sparse similarity: {} nodes, top-{} neighbors",
        sparse.n_points, sparse.k
    );
    ...
    println!(
        "Cluster 1 nodes have mostly cluster 1 neighbors: {}",
        cluster1_has_cluster1_neighbors
    );
    ...
    println!(
        "Dense matrix non-zeros: {} (expected ~{} = n*k)",
        nnz,
        n * k
    );
...
fn test_geodesic_distances() {
    ...
    assert!(
        north_to_north < north_to_equator,
...
fn test_entropy_weighted_optimization() {
    ...
    // Uniform prominence
    let prominence: Tensor<WgpuBackend, 1> = Tensor::from_data([1.0f32; 20].as_slice(), &device);
    ...
    assert!(
        corr_entropy < corr_no_entropy,
        "Entropy weighting should make correlation more negative: {} should be < {}",
        corr_entropy,
        corr_no_entropy
    );
