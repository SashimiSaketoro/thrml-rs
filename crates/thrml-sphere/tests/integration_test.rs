...
fn test_full_pipeline_with_blt_output() {
    ...
    let ebm =
        load_from_safetensors(&input_path, config, &device).expect("Failed to load SafeTensors");
    ...
    assert!(
        ebm.embedding_dim() > 0,
        "Embeddings should have non-zero dimension"
    );
    ...
    assert_eq!(
        coords.len(),
        ebm.n_points(),
        "Output should have same number of points"
    );
    ...
    println!("\n=== Results ===");
    println!(
        "Radii:  min={:.2}, max={:.2} (config: {:.2} - {:.2})",
        r_min, r_max, config.min_radius, config.max_radius
    );
    println!(
        "Theta:  min={:.4}, max={:.4} (valid: 0 - π ≈ 3.14)",
        theta_min, theta_max
    );
    println!(
        "Phi:    min={:.4}, max={:.4} (valid: 0 - 2π ≈ 6.28)",
        phi_min, phi_max
    );
    ...
    assert!(
        theta_min >= -0.5,
        "Theta should be >= 0 (got {})",
        theta_min
    );
    assert!(
        theta_max <= std::f32::consts::PI + 0.5,
        "Theta should be <= π (got {})",
        theta_max
    );
...
fn test_optimization_reduces_energy() {
    ...
    let ebm = load_from_safetensors(&input_path, config, &device).expect("Failed to load");

    // Get initial energy
    let init_coords = ebm.init_coords(&device);
    let hamiltonian = ebm.hamiltonian();
    let init_energy: Vec<f32> = hamiltonian
        .total_energy(&init_coords)
        .into_data()
        .to_vec()
    ...
    // Get final energy
    let final_energy: Vec<f32> = hamiltonian
        .total_energy(&final_coords)
        .into_data()
        .to_vec()
        .expect("energy to vec");
    ...
    println!(
        "Reduction: {:.2}%",
        (1.0 - final_total / init_total) * 100.0
    );
...
fn test_save_load_npz() {
    ...
    save_coords_npz(&coords, output_path).expect("Failed to save NPZ");
